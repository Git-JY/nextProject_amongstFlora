...
export async function queryExecute(str, value){
    ...
    const data = await new Promise((resolve, reject) => { // 이렇게 data로 넣은 후 보내도 됨
        connection.query(str, value, function(error, results){ // value는 str의 ?에 들어감
            resolve(results);  // 성공하면 resolve()의 인수 (ex. results)가 return // 실패하면 reject()의 인수가 return (ex. reject('실패') => '실패' 문자열 리턴)
        });
    });

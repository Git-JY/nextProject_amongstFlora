...
export default function Header({setSearchLoading, setItems}) {
  const elHeaderForm = useRef();
  const navigation = useRouter();

  const formActiveFun = () => {
    // console.log('elHeaderForm.current: ', elHeaderForm.current);
  ...
  const searchFun = async (e) => {
    ...
    let itemObj = await axios.post('/api/search', {data: data});
    setSearchLoading('');
    console.log('itemObj: ', itemObj);

    if(itemObj.data.totalCount._text == 1){ //검색 결과가 1개인 경우 // 배열이 아닌 객체로 줘서 이렇게 함
        let Oneitem = itemObj.data.item;

        setItems([Oneitem]);
    }else if(itemObj.data.totalCount._text == 0){ //검색 결과가 0개인 경우 // 아예 item이 없어서 이렇게 함
        setItems([]);

    }else{ // 검색결과가 2개 이상인 경우
        let itemArr = itemObj.data.item;
  ...
  const [checkedOption2, setCheckedOption2] = useState('0');
  const [checkedOption3, setCheckedOption3] = useState('0');
  const [checkedOption4, setCheckedOption4] = useState('0');
    ...
    <>
      <form className='searchAllOptionWrapper' ref={elHeaderForm} onSubmit={searchFun}>
          ...
          <div className='searchOptionScroll_hide'>
              ...
              <ul className='koreanChar_wrapper'>{/* 아직 찾는 법 못 찾음 */}
                  <li>
                      <input type='radio' name='koreanChar' value="1" id='giyeok'
                      checked={checkedOption2 === '1'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="giyeok">ㄱ</label>
                  </li>
                  
                  <li>
                      <input type='radio' name='koreanChar' value="2" id='nieun'
                      checked={checkedOption2 === '2'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="nieun">ㄴ</label>
                  </li>
                  
                  <li>
                      <input type='radio' name='koreanChar' value="3" id='digeut'
                      checked={checkedOption2 === '3'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="digeut">ㄷ</label>
                  </li>
                  
                  <li>
                      <input type='radio' name='koreanChar' value="4" id='rieul'
                      checked={checkedOption2 === '4'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="rieul">ㄹ</label>
                  </li>
                  
                  <li>
                      <input type='radio' name='koreanChar' value="5" id='mieum '
                      checked={checkedOption2 === '5'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="mieum ">ㅁ</label>
                  </li>
                  
                  <li>
                      <input type='radio' name='koreanChar' value="6" id='bieup'
                      checked={checkedOption2 === '6'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="bieup">ㅂ</label>
                  </li>

                  <li>
                      <input type='radio' name='koreanChar' value="7" id='siot'
                      checked={checkedOption2 === '7'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="siot">ㅅ</label>
                  </li>
                  
                  <li>
                      <input type='radio' name='koreanChar' value="8" id='ieung'
                      checked={checkedOption2 === '8'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="ieung">ㅇ</label>
                  </li>

                  <li>
                      <input type='radio' name='koreanChar' value="9" id='jieut'
                      checked={checkedOption2 === '9'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="jieut">ㅈ</label>
                  </li>
                  
                  <li>
                      <input type='radio' name='koreanChar' value="10" id='chieut'
                      checked={checkedOption2 === '10'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="chieut">ㅊ</label>
                  </li>
                  
                  <li>
                      <input type='radio' name='koreanChar' value="11" id='kieuk'
                      checked={checkedOption2 === '11'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="kieuk">ㅋ</label>
                  </li>

                  <li>
                      <input type='radio' name='koreanChar' value="12" id='tieut'
                      checked={checkedOption2 === '12'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="tieut">ㅌ</label>
                  </li>
                  
                  <li>
                      <input type='radio' name='koreanChar' value="13" id='pieup'
                      checked={checkedOption2 === '13'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="pieup">ㅍ</label>
                  </li>

                  <li>
                      <input type='radio' name='koreanChar' value="14" id='hieut'
                      checked={checkedOption2 === '14'}
                      onChange={(e) => {radioChange(e, 2)}}/>
                      <label htmlFor="hieut">ㅎ</label>
                  </li>

                  <li className='x'>
                      <input type='radio' name='koreanChar' value="0" id='x'
